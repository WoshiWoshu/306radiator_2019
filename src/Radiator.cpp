...
static void vecResult(const std::vector<std::vector<double>> &matrixA,
    std::vector<double> &matrixX, const std::vector<double> &matrixY,
    const int &nPow, const int &iterMax)
{
    double sum = 0;
...
static void computeCoeff(std::vector<std::vector<double>> &matrixA,
    const int &n, const int &nPow)
{
    double h = 0.5;
    double hPow = 1 / (h * h);
...
void radiator(int n, int nPow, int jr, int ir, int i, int j,
    const std::string &options)
{
    ...
    computeCoeff(matrixA, n, nPow);
    matrixY[jr * n + ir] = -300;
    vecResult(matrixA, matrixX, matrixY, nPow, 100);
    if (options == "A") {
        printCoeff(matrixA);
        printVecResult(matrixX);
    }
    if (options == "B") {
        printShortResult(matrixX, n, j, i);
    }
}
